...
server <- function(input, output) {
  ...
  output$main_plot <- renderPlot({
    ...
    ggplot(plot_df, aes(x = Wavelength, y = Reflectance, color = Type)) +
      ...
      theme_minimal() +
      labs(title = paste("Sample", input$sample_idx, "- Reference vs Reconstruction"),
  ...
  output$error_plot <- renderPlot({
    ...
    ggplot(df_err, aes(x = Wavelength, y = Error)) +
      ...
      theme_minimal() +
      labs(title = "Reconstruction Error (Reconstructed - Reference)",
  ...
  output$params_table <- renderTable({
    ...
    data.frame(
      Parameter = c("A (Chl)", "Sigma (Chl)", "A (Blue)", "Sigma (Blue)", 
